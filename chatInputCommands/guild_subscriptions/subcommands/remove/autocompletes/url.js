...
module.exports = new Autocomplete({
	...
	getRespond(interaction) {
		const channelId = interaction.options.getString("канал") ?? undefined;

		return getLinks({ subscriptions: { some: { guildId: interaction.guildId, targetId: channelId } } }).then((links) =>
			Promise.all(
